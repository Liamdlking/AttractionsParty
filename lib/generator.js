...
function buildAdditionalInfo(row) {
  ...
  if (food && String(food).trim())
    parts.push(`Food: ${String(food).trim()}`);

  if (notes && String(notes).trim())
    parts.push(`Notes: ${String(notes).trim()}`);

  if (tel && String(tel).trim())
    parts.push(`Tel: ${String(tel).trim()}`);
...
function groupByDate(rows, dateKey) {
  ...
  for (const r of rows) {
    const dateObj = parsePossibleDate(r[dateKey]);
...
async function generatePartySheets(rows, templatesDir) {
  ...
  const dateKey =
    sample["Date of Party"] != null ? "Date of Party"
    : sample["Party Date"] != null ? "Party Date"
    : "Date";

  const timeKey =
    sample["Party Start Time"] != null ? "Party Start Time"
    : sample["Party Time"] != null ? "Party Time"
    : "Time";
...
async function generateSigns(rows, templatesDir) {
  const tagTemplate = fs.readFileSync(
    path.join(templatesDir, "New Tag X Name Sign 2025.docx")
  );

  const stompTemplate = fs.readFileSync(
    path.join(templatesDir, "Stompers_Template_2PP.docx")
  );
  ...
  for (let i = 0; i < Math.max(1, tagNames.length); i += 4) {
    ...
    const buf = tagNames.length
      ? await replaceDocxPlaceholders(tagTemplate, mapping)
      : tagTemplate;

    tagFiles.push({
      name: `TagX_Signs_${tagFiles.length + 1}.docx`,
      buffer: buf
    });
  }
  ...
  for (let i = 0; i < Math.max(1, stompNames.length); i += 2) {
    ...
    const buf = stompNames.length
      ? await replaceDocxPlaceholders(stompTemplate, mapping)
      : stompTemplate;

    stompFiles.push({
      name: `Stompers_Signs_${stompFiles.length + 1}.docx`,
      buffer: buf
    });
  }
